    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form action="{{route('admin.password.update')}}" class="d-flex flex-column gap-2" method="POST" id="Change-Password">
                @csrf
                <input type="hidden" name="id" value="{{auth()->user()->id}}">
                <div>
                    <label for="current-password" class="form-label">Current Password:</label>
                    <input type="password" minlength="8" name="current_password" class="form-control password bg-transparent shadow validate" placeholder="Current Password" value="{{ old('current-password') }}">
