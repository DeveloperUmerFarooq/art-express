                    ...
                    <div class="card-header bg-success text-white rounded-top">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div class="text-right">
                                <div class="h5 mb-0 font-weight-bold">{{ number_format($order->items->sum('total_price')) }} Rs</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="collapse{{ $order->id }}" class="collapse show" aria-labelledby="heading{{ $order->id }}">
                        <div class="card-body">
                            <div class="d-flex flex-wrap mb-3">
                                <span class="badge badge-{{ $order->payment_status == 'Payed' ? 'success' : 'warning' }} badge-pill text-black mr-2 mb-2">
                                    <i class="{{ $order->payment_status == 'Payed' ? 'fas fa-check-circle' : 'fas fa-money-bill-wave' }} mr-1"></i>
                                ...
                                <span class="badge badge-{{ $order->status == 'cancelled' ? 'danger' : 'primary' }} badge-pill mr-2 mb-2 text-black">
                                    <i class="fas fa-{{ $order->status == 'cancelled' ? 'times-circle' : 'truck' }} mr-1"></i>
                                    ...
                                    <tbody>
                                        @foreach ($order->items as $item)
                                            <tr>
                                                ...
                                                <td class="text-right font-weight-bold">{{ number_format($item->total_price, 2) }} Rs</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            @can('cancel order')
                                @if (!auth()->user()->sales->contains('id',$order->id))
                                    <div class="d-flex gap-1 justify-content-end">
                                        <button class="btn btn-outline-danger" onclick="cancelOrder('{{ route('order.cancel', $order->id) }}')">
